...
int dp[20][4][2][2];
// nz-non zero
int dfs(int pos, int nz, int tight, int leadingzero)
{
    ...
    int &res = dp[pos][nz][tight][leadingzero];
    if (res != -1)
        return res;
    res = 0;
    ...
    for (int d = 0; d <= limit; d++)
    {
        ...
        int nextnz = nz + ((d == 0 && nextleadingzero) ? 0 : (d != 0));
        ...
        if (nextnz <= 3)
        {
            res += dfs(pos + 1, nextnz, nexttight, nextleadingzero);
        }
    }
    return dp[pos][nz][tight][leadingzero] = res;
}
int countUpto(int n)
{
    if (n < 0)
