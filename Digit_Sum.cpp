...
void solve()
{
    ...
    vector<vector<vector<int>>> dp(
        n + 1,
        vector<vector<int>>(2, vector<int>(D, 0)));
        ...
        for (int tight = 0; tight <= 1; tight++)
        {
            for (int sumMod = 0; sumMod < D; sumMod++)
            {
                ...
                for (int d = 0; d <= limit; d++)
                {
                    ...
                    dp[pos + 1][nextTight][nextSum] = (dp[pos + 1][nextTight][nextSum] + val) % MOD;
                }
            }
        }
    }
    int ans = (dp[n][0][0] + dp[n][1][0]) % MOD;
    ans = (ans - 1 + MOD) % MOD;
    cout << ans;
}
/*
